...
def signUpPosted(request):
	...
	if len(login_models.TEACHER_CODE_MAPPING.objects.filter(teacher_email=email_address)) > 0 and user_category == "STUDENT":
		return HttpResponse('''<body><script>alert("User specified email, is a teacher's email (as given to us by the school), so choose Teacher as your category.")</script><meta http-equiv="refresh" content="0; url='/signup/'"></body>''')
